...
const THEME_PROFILES = {
  ...
  'study-methods': {
    audience: [
      '社会人の学び直し層 (30-50代、資格・スキル取得)',
      '学習効率を上げたい大学生・大学院生',
      '企業の人材開発・研修担当',
      '学習アプリ・教材を作る個人開発者',
    ],
    monetization: [
      { title: '資格試験の学習コーチング', desc: '週次で進捗を確認し、計画を引き直す伴走型', barrier: '低', revenue: '月額 5,000-20,000円/人' },
      { title: '学習法の有料ニュースレター', desc: '研究論文ベースの学習法を週次で要約して配信', barrier: '低', revenue: '月額 500-1,500円' },
      { title: '企業向けリスキリング研修の設計', desc: '職種別の学習ロードマップと教材選定を代行', barrier: '中', revenue: 'プロジェクト 50-300万円' },
    ],
    content: [
      { format: 'YouTube', title: '「1年で資格3つ取った人の1日の使い方」', angle: '再現できる時間割まで見せる' },
      { format: 'Newsletter', title: '週次:「今週の学習科学」', angle: '論文 1 本を実践できる手順に翻訳する' },
      { format: 'ブログ', title: '「暗記が続かない人の共通点」', angle: '検索意図が明確な悩み系' },
    ],
    saas: [
      { title: '間隔反復の教材自動生成', target: '資格受験者', hypothesis: '教材はあるが、復習タイミングの設計を自分でやるのが続かない' },
      { title: '学習ログの可視化と週次レビュー', target: '社会人の学び直し層', hypothesis: '勉強時間は記録できても「何が身についたか」が見えない' },
    ],
  },
  'exam-admission': {
    audience: [
      '受験生本人 (中高生)',
      '受験生の保護者 (情報収集と費用判断をする側)',
      '塾・予備校の運営者',
      '教育系メディア・アフィリエイト運営者',
    ],
    monetization: [
      { title: 'オンライン個別指導', desc: '志望校別に週次で指導する少人数型', barrier: '低', revenue: '月額 15,000-40,000円/人' },
      { title: '志望校選びの相談サービス', desc: '模試結果と併願戦略を一緒に組み立てる', barrier: '低', revenue: '1回 5,000-20,000円' },
      { title: '受験情報の比較サイト運営', desc: '学費・倍率・進学実績を横断比較できる形にまとめる', barrier: '中', revenue: '広告・送客' },
    ],
    content: [
      { format: 'YouTube', title: '「併願校の決め方、模試の点数別」', angle: '判断基準を数字で出す' },
      { format: 'Newsletter', title: '月次:「入試制度の変更点まとめ」', angle: '保護者が見落としがちな変更を拾う' },
      { format: 'ブログ', title: '「受験にかかった費用、全部公開」', angle: '費用は検索需要が安定して高い' },
    ],
    saas: [
      { title: '併願パターンのシミュレーター', target: '受験生と保護者', hypothesis: '模試判定は出るが、組み合わせの妥当性は自分で判断できない' },
  ...
  'housing': {
    ...
    monetization: [
      { title: '住み替え相談（中立の立場）', desc: '売り手側につかず、買う・借りる側の条件整理を代行', barrier: '中', revenue: '1件 3-10万円' },
      { title: 'リノベ事例の取材メディア', desc: '実際の間取り・費用・失敗まで載せる事例データベース', barrier: '低', revenue: '広告・送客' },
      { title: '断熱改修の費用シミュレーション代行', desc: '補助金の条件と実費を突き合わせて試算する', barrier: '中', revenue: '1件 2-5万円' },
    ],
    content: [
      { format: 'YouTube', title: '「中古マンション、内見で見るべき12箇所」', angle: '現地で使えるチェックリストにする' },
      { format: 'Newsletter', title: '月次:「補助金と制度の変更点」', angle: '締切のある情報は再訪の理由になる' },
      { format: 'ブログ', title: '「リノベにかかった費用、全部公開」', angle: '費用は検索需要が安定して高い' },
    ],
    saas: [
      { title: '内見メモの共有アプリ', target: '住み替え世帯', hypothesis: '複数物件を見比べる時、家族間で条件がずれたまま進む' },
      { title: '補助金の適用可否チェック', target: 'リフォーム事業者', hypothesis: '制度が自治体ごとに違い、確認が手作業になっている' },
    ],
  },
  'home-appliance': {
    audience: [
    ...
    monetization: [
      { title: '買い替え相談（型番まで指定）', desc: '間取りと使い方を聞いて機種を絞り込む', barrier: '低', revenue: '1件 3,000-10,000円' },
      { title: '実測レビューのメディア運営', desc: 'カタログ値ではなく自分で測った電気代・騒音を出す', barrier: '低', revenue: '広告・アフィリエイト' },
      { title: '法人向けの什器・家電の選定代行', desc: 'オフィスや店舗の設備選定と見積比較', barrier: '中', revenue: 'プロジェクト 10-50万円' },
    ],
    content: [
      { format: 'YouTube', title: '「エアコンの電気代、実測で比べた」', angle: 'カタログ値ではなく自分で測る' },
      { format: 'Newsletter', title: '週次:「今週の値下がり家電」', angle: '価格は動くので再訪の理由になる' },
      { format: 'ブログ', title: '「買って後悔した家電と、その理由」', angle: '失敗談は検索されやすい' },
    ],
    saas: [
      { title: '家電の買い替え時期の通知', target: '一般世帯', hypothesis: '寿命が来てから慌てて選び、比較する時間がない' },
      { title: '電気代の内訳を機器別に推定', target: '電気代を下げたい世帯', hypothesis: '総額は分かっても、どの機器が食っているかは分からない' },
    ],
  },
  'ai-regulation': {
    audience: [
