    ...
    <div className="container compare-page">
      ...
      {(!demandA || !demandB) && (
        <>
          ...
          {/* 2026-08-01 実測: 選ぶ前のこのページは本文 429 字・内部リンク 0 本で、
              検索から来た人には「何のページか」も「次にどこへ行けるか」も
              分からない行き止まりだった。説明と入口を置く。 */}
          <div className="prose cmp-explainer">
            <h2>並べて見ると何が分かるか</h2>
            <p>
              需要スコアは 1 つだけ見ても意味がありません。80 点が高いのか低いのかは、
              他と並べて初めて分かります。ここでは 2 つのテーマについて、
              スコア・前日比・勢い・参入しやすさ・競争・情報源ごとの観測量・
              事業アイデアを同じ並びで表示します。
            </p>
            <p>
              とくに<strong>情報源ごとの内訳</strong>を見比べると、
              同じスコアでも中身が違うことが分かります。
              報道だけが多いテーマと、研究も実装も動いているテーマでは、
              取るべき行動が変わります。
            </p>

            <h2>選び方のヒント</h2>
            <ul>
              <li><strong>迷っている 2 つを直接並べる</strong> — どちらに時間を使うかを決めるとき</li>
              <li><strong>同じ分野の 1 位と 2 位を並べる</strong> — その分野で何が差を生んでいるかを見るとき</li>
              <li><strong>判定が違う 2 つを並べる</strong> — 「拡大局面」と「研究先行」の差を体感するとき</li>
            </ul>

            <h2>上位のテーマから選ぶ</h2>
            <div className="cmp-quick-links">
              {demands.slice(0, 6).map((d) => (
                <Link key={d.id} to={`/demand/${d.id}`} className="cmp-quick-link">
                  {d.title} <span>{d.score}</span>
                </Link>
              ))}
            </div>
            <p>
              数字の意味は<Link to="/glossary">用語集</Link>、
              読む順番は<Link to="/guide">このサイトの読み方</Link>にまとめています。
            </p>
          </div>
        </>
      )}
